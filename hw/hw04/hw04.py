...
def replace_leaf(t, find_value, replace_value):
    ...
    for i in range(len(branch)):
        if(is_leaf(branch[i]) and label(branch[i])==find_value):
            branch[i]=tree(replace_value)
        else:
...
#    def check(t=t,phrase=phrase,res='',depth=0):
#        res+=label(t)
#        depth+=1
#        if(depth < len(phrase) and res[depth-1] != phrase[depth-1]):
#            return False
#        elif(depth == len(phrase) and res == phrase):
#            return True
#        elif(depth == len(phrase) and res != phrase):
#            return False
#        else:
#            for branch in branches(t):
#                if(check(branch,res=res,depth=depth)):
#                    return True
#            return False
#    return check()
...
def quadratic(x, a, b, c):
    ...
    middle = b/-2/a if b/-2/a>lower_bound(x) and b/-2/a<upper_bound(x) else lower_bound(x)
    middle=function(middle)
    left=function(lower_bound(x))
    right=function(upper_bound(x))
    return interval(min(middle,left,right),max(middle,left,right))
