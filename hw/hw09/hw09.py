...
def in_order_traversal(t):
    ...
    if(t.is_leaf()):
        ...
        for i in in_order_traversal(t.branches[0]):
            yield i
        yield t.label
        for i in in_order_traversal(t.branches[1]):
            yield i
...
def interleaved_sum(n, odd_term, even_term):
    ...
    "*** YOUR CODE HERE ***"
    sum=0
    def func(num,odd_or_even):
        nonlocal sum
        sum+=odd_term(num) if odd_or_even else even_term(num)
        if(num==n):
            return
        func(num+1,0 if odd_or_even else 1)
    func(1,1)
    return sum
...
def mutate_reverse(link):
    ...
    data=deepcopy(link)
    while(data!=Link.empty):
        tmp=head.rest
        head.rest=Link(data.first)
        head.rest.rest=tmp
        data=data.rest
    link.first=head.rest.first
    link.rest=head.rest.rest
