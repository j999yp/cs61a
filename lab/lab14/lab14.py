...
def align_skeleton(skeleton, code):
    ...
    def helper_align(skeleton_idx, code_idx):
        ...
        if skel_char == code_char:
            char=skel_char
            tmp,cost=helper_align(skeleton_idx+1,code_idx+1)
            possibilities.append((char+tmp,cost))
        # Insert
        edits="+["+code[code_idx]+"]"
        tmp,cost=helper_align(skeleton_idx,code_idx+1)
        possibilities.append((edits+tmp,cost+1))
        # Delete
        edits="-["+skeleton[skeleton_idx]+"]"
        tmp,cost=helper_align(skeleton_idx+1,code_idx)
        possibilities.append((edits+tmp,cost+1))
...
def num_splits(s, d):
    ...
    def func(left,right,n):
        ...
        if(n==len(s)):
            ...
            func(left+s[n],right,n+1)
            func(left,right+s[n],n+1)
    func(0,0,0)
    return cnt//2
...
def insert(link, value, index):
    ...
    if(index==0):
        tmp=deepcopy(link)
        link.first,link.rest=value,tmp
