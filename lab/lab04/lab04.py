...
def reverse_recursive(lst):
    ...
    def func(index,res=[]):
        if(index==-1):
            ...
            return func(index-1,res+[lst[index]])
    return func(index=cnt)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    return get_name(city_a) if distance(tmp,city_a) < distance(tmp,city_b) else get_name(city_b)
...
def add_chars(w1, w2):
    ...
    def func(s1,s2,res=''):
        if(not s1):
            ...
            return func(s1[1:],s2[1:],res)
        else:
            return func(s1,s2[1:],res+s2[0])
    return func(w1,w2)
