...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while(current_place!=beehive):
            if(current_place.bees and cnt<=self.max_range and cnt>=self.min_range):
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if(not self.die_round_later):
            QueenAnt.die_round_later=True
            self.die_round_later=False
        super().__init__(armor)
        # END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        self.armor -= amount
        if self.armor <= 0 and not self.die_round_later:
            bees_win()
        # END Problem 13
    def remove_from(self, place):
        if(self.die_round_later):
...
class Bee(Insect):
    ...
    def action(self, gamestate):
        ...
        if(self.direction=='left'):
            ...
            destination = self.place.entrance if type(self.place.entrance)!=Hive else None
        # END EC
        if self.blocked():
...
############
bee_status={}
def new_action_constructor(action,bee):
    def new_action(*args):
        # get gamestate
        global gamestate,bee_status
        try:
            gamestate=gamestate
        except:
            import gc
            for obj in gc.get_objects():
                if isinstance(obj, GameState):
                    ...
                    break
        # check all debuffs
        if_scare='make_scare' in [i[0] for i in bee_status[bee]]
        bee.direction='right' if if_scare else 'left'
        bee.scared=if_scare if bee.scared==False else True

        if_stuck='make_slow' in [i[0] for i in bee_status[bee]]
        if(if_stuck and gamestate.time%2==1):
            stuck=False
            for debuff in bee_status[bee]:
                if(debuff[0]=='make_scare'):
                    debuff[1]-=1
                if(debuff[0]=='make_slow' and not stuck):
                    debuff[1]-=1
                    stuck=True
        else:
            action(*args)
            for debuff in bee_status[bee]:
                debuff[1]-=1
        bee_status[bee]=[i for i in bee_status[bee] if i[1]!=0]
    return new_action
...
def make_slow(action, bee):
    ...
    # BEGIN Problem EC
    "*** YOUR CODE HERE ***"
    return new_action_constructor(action,bee)
    # END Problem EC
...
def apply_status(status, bee, length):
    ...
    "*** YOUR CODE HERE ***"
    global bee_status
    if((status.__name__ == 'make_scare' and bee.scared == False) or status.__name__ == 'make_slow'):
        if bee in bee_status:
            bee_status[bee].append([status.__name__,length])
        else:
            bee_status[bee]=[[status.__name__,length]]
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self, beehive):
        ...
        distance=0
        while(current_place!=beehive):
            insects=current_place.bees+[current_place.ant]
            for insect in [i for i in insects if i]:
                res[insect]=distance
            distance+=1
            current_place=current_place.entrance
        del res[self]
